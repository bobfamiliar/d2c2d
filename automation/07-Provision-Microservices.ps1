...
$command = $Path + "\microservices\provision\automation\01-Provision-ProvisionM.ps1"
&$command -repo $Path -Subscription $Subscription -ResourceGroup $ResourceGroup -AzureLocation $AzureLocation -Prefix $Prefix -Suffix $Suffix
...
$command = $Path + "\microservices\provision\automation\02-Build-ProvisionM.ps1"
&$command -repo $Path -configuration $Configuration

$command = $Path + "\microservices\provision\automation\03-Deploy-ProvisionM.ps1"
