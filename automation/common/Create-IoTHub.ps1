<#
.Synopsis 
    This PowerShell script provisions an instance of Azure IoTHub
.Description 
    This PowerShell script provisions an instance of Azure IoTHub
.Notes 
    File Name  : Create-IoTHub.ps1
    Author     : Bob Familiar
    Requires   : PowerShell V4 or above, PowerShell / ISE Elevated

    Please do not forget to ensure you have the proper local PowerShell Execution Policy set:

        Example:  Set-ExecutionPolicy Unrestricted 

    NEED HELP?

    Get-Help .\Create-IoTHub.ps1 [Null], [-Full], [-Detailed], [-Examples]

.Link   
    https://microservices.codeplex.com/

.Parameter Repo
    Example:  c:\users\bob\source\repos\looksfamiliar
.Parameter Subscription
    Example:  mysubscription
.Parameter DocDbName
    Example:  myIoTHub
.Parameter ResourceGroupName
    Example:  MyResourceGroup
.Parameter AzureLocation
    Example:  East US
.Example
    .\Create-IoTHub.ps1 -repo "c:\users\bob\source\repos\looksfamiliar" -Subscription mysubscription -IoTHubName myIoTHub -ResourceGroupName MyResourceGroup -AzureLocation "East US"
.Inputs
    The [Repo] parameter is the path to the top level folder of the Git Repo.
    The [Subscription] parameter is the name of the Azure subscription.
    The [IoTHubName] parameter is the IoTHub name.
    The [ResourceGroupName] parameter is the name of the Azure Resource Group
    The [AzureLocation] parameter is the name of the Azure Region/Location: East US, Central US, West US.
.Outputs
    Console
#>

[CmdletBinding()]
Param(
    [Parameter(Mandatory=$True, Position=0, HelpMessage="The path to the Git Repo.")]
    [string]$Repo,
    [Parameter(Mandatory=$True, Position=1, HelpMessage="The subsription name.")]
    [string]$Subscription,
    [Parameter(Mandatory=$True, Position=2, HelpMessage="The IoTHub name.")]
    [string]$IoTHubName,
    [Parameter(Mandatory=$True, Position=3, HelpMessage="The resource group name")]
    [string]$ResourceGroupName,
    [Parameter(Mandatory=$True, Position=4, HelpMessage="The name of the Azure Region/Location: East US, Central US, West US.")]
    [string]$AzureLocation
)

#######################################################################################
# F U N C T I O N S
#######################################################################################

Function Select-Subscription()
{
    Param([String] $Subscription)

    Try
    {
        Set-AzureRmContext  -SubscriptionName $Subscription -ErrorAction Stop
    }
    Catch
    {
        Write-Verbose -Message $Error[0].Exception.Message
    }
}

Function Create-IoTHub
{
    Param([String] $Repo, [String] $Name, [String] $Group, [String] $Location)

    Try
    {

        $JSON = @"
{
        "$schema": "http://schema.management.azure.com/schemas/2015-01-01/deploymentTemplate.json#",
    "contentVersion": "1.0.0.0",
    "parameters": {
        "iotHubName": {
            "value": "$Name"
        },
        "location": {
            "value": "$Location"
        }
    }
}
"@
        $ParamsPath = $Repo + "\Automation\Templates\iothub-template-params.json"
        $TemplatePath = $Repo + "\Automation\Templates\iothub-template.json"

        $JSON | Set-Content -Path $ParamsPath

        New-AzureRmResourceGroupDeployment -ResourceGroupName $Group -TemplateFile $TemplatePath -TemplateParameterFile $ParamsPath
    }
    Catch
    {
        Write-Verbose -Message $Error[0].Exception.Message
        Write-Verbose -Message "Exiting due to exception: IoTHub Not Created."
    }
}

##########################################################################################
# M A I N
##########################################################################################

$Error.Clear()

# Mark the start time.
$StartTime = Get-Date

# Select Subscription
Select-Subscription $Subscription

# Create IoT Hub
Create-IoTHub -Repo $Repo -Name $IoTHubName -Group $ResourceGroupName -Location $AzureLocation

# Mark the finish time.
$FinishTime = Get-Date

#Console output
$TotalTime = ($FinishTime - $StartTime).TotalSeconds
Write-Verbose -Message "Elapse Time (Seconds): $TotalTime"
